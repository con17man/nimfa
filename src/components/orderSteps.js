  ...
  return(
    <div className="order-steps w-full py-12">
      <div className="steps-list container mx-auto text-center">
        <p className="list-title font-montserrat-alternates font-semibold text-4xl">{orderStepsData.title}</p>
        <p className="list-description font-light mx-auto max-w-2xl">{orderStepsData.description}</p>
        <div className="list-items flex mx-auto max-w-2xl">
          {orderStepsData.steps.map((step, i) => {
            return  <div key={i+1}
                      className="list-item adv-item-img h-32 w-32 my-6 rounded-full shadow-xl mx-auto bg-red flex justify-center items-center text-5xl text-white">
